-- ============================================================================
-- MIGRATION: Add Infisical Support (CORRECTED FOR YOUR SCHEMA)
-- ============================================================================
-- Description: Adds columns to track which projects use Infisical for key storage
-- Your schema uses vault_keys table (separate) instead of vault_private_key_encrypted column
-- 
-- Run with Supabase SQL Editor:
-- https://supabase.com/dashboard/project/irxfajjcwhdgamzqqfyf/sql
-- ============================================================================

-- Step 1: Add flag to track migration status in projects table
ALTER TABLE projects 
ADD COLUMN IF NOT EXISTS uses_infisical BOOLEAN DEFAULT FALSE;

COMMENT ON COLUMN projects.uses_infisical IS 
  'TRUE if project uses Infisical for key storage, FALSE if using vault_keys table';
...
COMMENT ON TABLE vault_access_log IS 
  'Audit trail for all vault key access attempts (both vault_keys table and Infisical)';
...
-- Step 5: Set existing projects to use vault_keys table (database encryption)
-- Projects with entries in vault_keys table are using database encryption
UPDATE projects 
SET uses_infisical = FALSE 
WHERE id IN (SELECT project_id FROM vault_keys)
  AND (uses_infisical IS NULL OR uses_infisical = TRUE);
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE uses_infisical = FALSE) as using_vault_keys,
  COUNT(*) FILTER (WHERE p.id IN (SELECT project_id FROM vault_keys)) as has_vault_key_backup,
  ROUND(
...
COMMENT ON VIEW migration_progress IS 
  'Shows migration progress from vault_keys table to Infisical';
...
-- Show which projects have vault_keys entries
SELECT 
  p.name,
  p.uses_infisical,
  vk.project_id IS NOT NULL as has_vault_key
FROM projects p
LEFT JOIN vault_keys vk ON vk.project_id = p.id
