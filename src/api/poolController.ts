...
export class PoolController {
      ...
      if (tokenMintToCheck) {
        ...
        if (isNativeSOL) {
          ...
          if (floatLessThan(solBalanceInSOL, requiredSOL, EPSILON)) {
            ...
            result.checks.tokenBalance.message = `Native SOL balance sufficient: ${formatTokenAmount(solBalanceInSOL, 4)} SOL (need ${formatTokenAmount(requiredSOL, 4)} SOL)`;
          }
        } else {
          // SPL Token - check token account
          try {
  ...
  async createPool(req: Request, res: Response) {
    try {
      ...
      // Get token_mint from request body - REQUIRED, no fallback
      const poolTokenMint = req.body.token_mint;
      
      if (!poolTokenMint) {
        return res.status(400).json({
          error: 'token_mint is required',
          message: 'You must select a token when creating a pool. Please select a token from the dropdown in Step 1.',
        });
      }
      ...
      if (!skipStreamflow) {
        try {
          ...
          if (isNativeSOL) {
            ...
            // MUST match streamflowService.ts and validation calculation
            const solBalance = await this.connection.getBalance(adminKeypair.publicKey);
            const solBalanceInSOL = solBalance / LAMPORTS_PER_SOL;
            
            // Use EXACT same calculation as streamflowService.ts (line 77) and validation (line 204)
            const poolWithStreamflowFee = total_pool_amount * 1.005; // 0.5% Streamflow fee
            const wrapAndRentFees = 0.02; // Wrapping tx + account rent + buffer
            const requiredSOL = poolWithStreamflowFee + wrapAndRentFees;
            
            console.log(`[STREAMFLOW] Native SOL pool - Treasury balance check:`);
            console.log(`[STREAMFLOW]   Pool amount: ${total_pool_amount} SOL`);
            ...
            console.log(`[STREAMFLOW]   Treasury balance: ${solBalanceInSOL.toFixed(4)} SOL`);
            ...
            console.log('[STREAMFLOW] ✅ Treasury has sufficient SOL for deployment');
          } else {
            // For SPL tokens, check both token balance and SOL for fees
            ...
            try {
              ...
              // Use consistent epsilon-based comparison
              if (floatLessThan(tokenBalance, requiredTokens, EPSILON)) {
                throw new Error(`Treasury has insufficient tokens. Required: ${formatTokenAmount(requiredTokens, 2)}, Available: ${formatTokenAmount(tokenBalance, 2)}. Please fund the treasury before deploying.`);
              }
              console.log('[STREAMFLOW] ✅ Treasury has sufficient tokens');
            } catch (err) {
              if (err instanceof Error && err.message.includes('could not find account')) {
                throw new Error(`Treasury token account does not exist. Please create and fund the token account before deploying.`);
              }
              throw err;
            }
            
            // Also check SOL for deployment fees
            const solBalance = await this.connection.getBalance(adminKeypair.publicKey);
            const solBalanceInSOL = solBalance / LAMPORTS_PER_SOL;
          ...
          // PRE-FLIGHT VALIDATION: Validate parameters before any blockchain interaction
          console.log('[STREAMFLOW] Running parameter validation...');
          const paramErrors = validatePoolParameters({
          ...
          console.log('[STREAMFLOW] ✅ Parameters validated');

          // Calculate effective timestamps BEFORE simulation
          const originalDuration = endTimestamp - startTimestamp;
          const simStartTime = startTimestamp < nowTimestamp ? nowTimestamp + 60 : startTimestamp;
          const simEndTime = startTimestamp < nowTimestamp ? simStartTime + originalDuration : endTimestamp;
          
          console.log('[STREAMFLOW] Timestamp calculation:');
          console.log(`[STREAMFLOW]   Original: start=${startTimestamp}, end=${endTimestamp}, duration=${originalDuration}s`);
          console.log(`[STREAMFLOW]   Effective: start=${simStartTime}, end=${simEndTime}`);
          console.log(`[STREAMFLOW]   Now: ${nowTimestamp}`);
          
          // Validate timestamps BEFORE simulation
          if (simEndTime <= simStartTime) {
            throw new Error(
              `Invalid timestamps: end time (${simEndTime}) must be after start time (${simStartTime}). ` +
              `Duration was ${originalDuration}s. Please set a future start time or longer duration.`
            );
          }

          // PRE-FLIGHT SIMULATION: Simulate the transaction before execution
          console.log('[STREAMFLOW] Running pre-flight simulation...');
          const simulation = await simulateStreamflowPoolCreation(this.connection, {
            treasuryKeypair: adminKeypair,
            tokenMint: tokenMint,
            totalAmount: poolData.total_pool_amount,
          ...
          if (!simulation.success) {
            console.error('[STREAMFLOW] ❌ Simulation failed:', simulation.errors);
            throw new Error(`Pre-flight simulation failed: ${simulation.errors.join(', ')}`);
          }

          if (simulation.warnings.length > 0) {
            console.warn('[STREAMFLOW] ⚠️  Simulation warnings:', simulation.warnings);
          }

          console.log('[STREAMFLOW] ✅ Simulation passed');
          console.log('[STREAMFLOW] Balances:', {
            sol: `${formatTokenAmount(simulation.balances.sol, 4)} SOL`,
            tokens: `${formatTokenAmount(simulation.balances.tokens, 4)} tokens`,
          });

          // Calculate effective timestamps (handle past start times)
          let effectiveStart = startTimestamp;
          let effectiveEnd = endTimestamp;
          const duration = endTimestamp - startTimestamp;
          
          if (startTimestamp < nowTimestamp) {
            console.warn(`[STREAMFLOW] Start time ${startTimestamp} is in the past (now: ${nowTimestamp}). Adjusting...`);
            effectiveStart = nowTimestamp + 60;
            effectiveEnd = effectiveStart + duration;
            console.log(`[STREAMFLOW] Adjusted: start=${effectiveStart}, end=${effectiveEnd}, duration=${duration}s`);
          }

          // CRITICAL: Validate that end time is AFTER start time
          if (effectiveEnd <= effectiveStart) {
            throw new Error(
              `Invalid pool duration: End time (${effectiveEnd}) must be after start time (${effectiveStart}). ` +
              `Original: start=${startTimestamp}, end=${endTimestamp}, duration=${duration}s. ` +
              `Please increase the vesting duration.`
            );
          }

          // Validate timestamps for Streamflow
          if (startTimestamp < nowTimestamp) {
            const adjustedStart = effectiveStart;
            const adjustedEnd = effectiveEnd;
